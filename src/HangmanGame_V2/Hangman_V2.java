...
public class Hangman_V2 {
    ...
    StringBuffer outputString; // 플레이어의 입력에 따른 결과로 보여주는 문자열
    StringBuffer inputString; // 플레이어가 입력한 문자들의 모임
    int remainder; // 맞출 문자수 ( 못 맞추고 남아있는 문자의 수)
    int failed; // 실패한 횟수

    public Hangman_V2() throws IOException {
    ...
    public int playGame() throws IOException {
        ...
        } while ((remainder > 0) && (failed < 6)); // 문제를 완전히 맞추거나 6번 이상 틀릴 때 까지 반복
